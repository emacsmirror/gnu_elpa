...
- User Option: transient-enable-menu-navigation ::
  ...
  {{{kbd(mouse-1)}}} and {{{kbd(mouse-2)}}} are bound in ~transient-button-map~.
  All other bindings are in ~transient-popup-navigation-map~.

  Instead of {{{kbd(RET)}}}, {{{kbd(M-RET)}}} is used to invoke the suffix command at point by
...
- User Option: transient-navigate-to-group-descriptions ::
  ...
  default, {{{kbd(UP)}}} and {{{kbd(DOWN)}}} move from suffix to suffix.  When this option
  is non-~nil~ as well, then they additionally stop at group descriptions.
...
- User Option: transient-force-fixed-pitch ::
  ...
  in menu buffer.  Even if you use a proportional font for the ~default~
  face, you might still want to use a monospaced font in the menu
  buffer.  Setting this option to ~t~ causes ~default~ to be remapped to
  ~fixed-pitch~ in that buffer.
...
- {{{var(LOC)}}} is a key description (a string as returned by ~key-description~
...
~magit-diff~ (~d~) the same change also applies to ~magit-diff-refresh~ (~D~).
...
command.  Magit's ~magit-dispatch~ (on {{{kbd(C-x M-g)}}}) command is an example of
...
For example, while ~magit-diff~ (~d~) and ~magit-diff-refresh~ (~D~) offer
...
Such shared groups should be defined using ~transient-define-group~
...
- Macro: transient-define-group name group... ::
  ...
  GROUPs have the same form as for ~transient-define-prefix~.
...
- Function: transient-show-summary obj &optional return ::
  ...
  (if the value of ~transient-enable-menu-navigation~ is ~verbose~) when
...
** Can I control how the menu buffer is displayed?
:PROPERTIES:
:UNNUMBERED: notoc
:END:
...
** How can I copy text from the menu buffer?
:PROPERTIES:
:UNNUMBERED: notoc
:END:
