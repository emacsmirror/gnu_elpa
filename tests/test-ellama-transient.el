...
(ert-deftest test-ellama-transient-provider-candidates-hide-provider-values ()
  (let* ((secret "transient-secret")
         (ellama-provider (make-llm-openai-compatible
                           ...
                           :chat-model "model"
                           :key secret))
         (ellama-provider-list '(ellama-provider))
         captured)
    (cl-letf (((symbol-function 'completing-read)
               (lambda (_prompt collection &rest _args)
                 (setq captured collection)
                 (caar collection))))
      (should (eq (ellama-transient--read-provider-symbol) 'ellama-provider))
    ...
    (cl-letf (((symbol-function 'completing-read)
              ...
              ((symbol-function 'ellama-construct-provider-from-transient)
               (lambda (&rest args)
                 ...
                 :new-provider)))
      (ellama-transient-set-provider)
      (should-not received-args)
      (should (eq ellama-provider :new-provider)))))
