...
- ~cl-ts-mode-format-indent-parent-predicate~ :: A predicate for
  ~treesit-node-match-p~ (usually a function or regexp matching a treesit node's
  type) to determine which nodes should have their contents indented. By default
  it matches the four paired directives, but you can also set it to indent
  relative to the start of the entire format string.
- ~cl-ts-mode-format-indent-align-block-ends~ :: Controls indentation of the end
  of a paired directive when it begins a line. If non-nil (the default), the
  directive characters themselves are aligned, like
  #+begin_src lisp :tangle yes
    "~:@{
       ~}"
  #+end_src
