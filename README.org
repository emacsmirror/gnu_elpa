...
=doc-view-follow.el= is available on ELPA, so you can install it from there via =M-x packages-install=.  Alternatively, install it via your preferred package manager, or download =doc-view-follow.el= and run =M-x package-install-file=.
