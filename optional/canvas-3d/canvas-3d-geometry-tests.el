...
(defun canvas-3d-geometry-test--color ()
  "Read the actual displayed native backing bytes for comparison."
  ...
  (dolist (target '(((mesh . "model") (kind . "point") (face . 0)
    ...
    (with-temp-buffer
      (cl-letf (((symbol-function 'display-graphic-p) (lambda (&rest _) t))
        ...
        (let* ((original (canvas-3d-geometry-test--color))
               (pixel (+ 64 (* 64 128)))
               (seq canvas-3d--seq)
               events)
          ...
          (should (= canvas-3d--seq (+ 2 seq)))
          (canvas-3d-geometry-test--settle)
          (should (equal original (canvas-3d-geometry-test--color)))
          (should (equal target canvas-3d--question-target))
          (canvas-3d-reveal)
          (should-not (string-match-p "SECRET" (canvas-3d--header)))
          (canvas-3d-right)
          (canvas-3d-geometry-test--settle)
          (should-not (equal original (canvas-3d-geometry-test--color)))
          (canvas-3d-zoom-in)
          (canvas-3d-geometry-test--settle)
          (canvas-3d-reset)
          (canvas-3d-geometry-test--settle)
