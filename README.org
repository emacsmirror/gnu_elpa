...
implement a review process for some of your [[ https://protesilaos.com/emacs/denote][denote]] notes.
...
- ~denote-review-set-date-dired-marked-files~:
  Insert the creation date or the current date as review date in notes marked in Dired.
...
the useroption ~denote-review-insert-after~.
...
`denote-review' uses the `re-search-forward' command to search
