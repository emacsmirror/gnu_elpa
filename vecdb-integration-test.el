  ...
  (let ((chroma-test-name (intern (format "%s-chroma" base-name)))
    ...
    `(progn
       ...
       (ert-deftest ,psql-test-name ()
         ...
         (interactive)
         (let ((current-provider (cl-find-if (lambda (p) (eq (type-of p) 'vecdb-psql-provider))
                                             (vecdb-test--get-providers))))
           (if current-provider
               (funcall ,body-function current-provider)
             (ert-skip (format "Postgres provider not configured for %s" ',psql-test-name))))))))
  ...
  (let ((full-collection-name (gensym "full-collection-name-"))
    ...
    `(let* ((,full-collection-name (format "%s-%s" ,collection-name-base (vecdb-provider-name ,current-provider)))
            (,collection-var (make-vecdb-collection :name ,full-collection-name
                                                    :vector-size (or (plist-get ,options :vector-size) ,default-vector-size)
                                                    :payload-fields (plist-get ,options :payload-fields))))
       (unwind-protect
           (progn
