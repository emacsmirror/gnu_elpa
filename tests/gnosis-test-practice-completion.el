  ...
  (gnosis-test-completion
    ...
    (let ((target (gnosis-review--session-target))
      ...
      (dolist (failure '(decline quit error setup))
        (ert-info ((symbol-name failure))
          (let ((prompt-count 0) (setup-count 0) outcome)
            (cl-letf (((symbol-function 'y-or-n-p)
                       (lambda (_)
                      ...
                      ((symbol-function 'gnosis-review--setup-buffer)
                       (lambda (&rest _)
                         (cl-incf setup-count)
                         ;; Bound setup even if a declined prompt is ignored.
                         (error "Setup failed"))))
              (setq outcome
                    (condition-case err (gnosis-review-summary-scheduled)
                      ((error quit) err))))
            (should (= prompt-count 1))
            (should (= setup-count (if (eq failure 'setup) 1 0)))
            (should (equal outcome (pcase failure
                                     ('decline nil) ('quit '(quit))
