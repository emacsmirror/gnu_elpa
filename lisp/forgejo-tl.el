...
;;
;; Vendored from gnosis-tl.el (part of gnosis.el).
;;
...
;;    column per entry).  Instead, `forgejo-tl--column-at-point'
...
(defun forgejo-tl--column-specs (format)
...
(defun forgejo-tl--format-line (id cols specs)
  "Build a propertized line string for entry ID with COLS and SPECS.
COLS is the entry's column vector.  SPECS is from `forgejo-tl--column-specs'.
...
(defun forgejo-tl--column-at-point ()
...
(defun forgejo-tl--render-into-buffer (entries format padding)
...
`tabulated-list-id', `tabulated-list-entry'."
  (let* ((specs (forgejo-tl--column-specs format))
         (n-cols (length specs))
         (last-idx (1- n-cols))
         (ellipsis forgejo-tl-ellipsis)
...
(defun forgejo-tl-render-lines (entries format padding)
...
of all formatted lines.  Pure function -- no buffer side effects."
  (with-temp-buffer
    (let ((inhibit-modification-hooks t))
      (forgejo-tl--render-into-buffer entries format padding))
    (buffer-string)))

(defun forgejo-tl-print (&optional remember-pos)
...
Like `tabulated-list-sort' but re-renders with `forgejo-tl-print'.
...
(defun forgejo-tl-replace-entry (id new-cols)
...
Point is preserved via `save-excursion'."
  (let ((inhibit-read-only t)
        (specs (forgejo-tl--column-specs tabulated-list-format)))
    (save-excursion
      (goto-char (point-min))
      (while (and (not (eobp))
      ...
      (when (and (not (eobp))
        ...
        (let ((beg (line-beginning-position))
          ...
          (goto-char beg)
          (insert (forgejo-tl--format-line id new-cols specs)))))))

(defun forgejo-tl-delete-entry (id)
...
(defun forgejo-tl-append-entries (entries)
  "Append ENTRIES at the end of the current tabulated-list buffer.
Uses `forgejo-tl--render-into-buffer' at `point-max'.
