...
#+MACRO: version 1.6.0
#+MACRO: updated last updated 3 May 2026
...
| Command                                  | Default key | Comment out current... |
|------------------------------------------+-------------+------------------------|
...
+ ~latex-table-wizard-after-table-modified-hook~: run after each table
  changing operation done by a ~latex-table-wizard~ function
+ ~latex-table-wizard-after-movement-hook~: run after each
  ~latex-table-wizard~ movement command (that is, a command whose
  purpose it to move point inside the table)

A function that is a natural fit for these hooks, could be, for
example, one of the ~latex-table-wizard-align~ commands.
