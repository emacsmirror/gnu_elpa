...
- Defining custom per-project commands in the menu.  Mechanisms are provided to
  make it easy to integrate with ~compile~ (e.g. automatic buffer naming) or run
  custom elisp code.  See ~disproject-custom-suffixes~ for documentation.
