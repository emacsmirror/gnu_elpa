...
- A grammar for FORMAT directives :: This is a multi grammar repo, containing a
  ...
  #+end_example
I had to use an [[https://tree-sitter.github.io/tree-sitter/creating-parsers/4-external-scanners.html][external scanner]] for both symbol and format directive parsing. [[file:common/scanner.h][My C isn't great]], feel free to let me know what I'm doing wrong.
...
certainly change. I didn't use "cl-parse" because I didn't want the prefix to
clash with ~cl-lib~. I'm tentatively thinking "semantic-cl", or maybe
"cl-scope".
...
boxes in the screenshot are from an additional minor mode which highlights local
bindings at point.
...
Or more simply
#+begin_src elisp :tangle yes :lexical yes
  (get (clparse-canonize-node NODE) :definition)
#+end_src
I'm not sure why I went with the term "canonize" but it's now used throughout.
...
the value is ~(DEPTH . LOCALS)~, where each of ~LOCALS~ is ~(CANONIZED-SYMBOL
TREESIT-NODE . FACE-LIST)~. Storing all of this in a single variable is quite
handy:
